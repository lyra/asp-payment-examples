...
namespace core.Services
{
    public class PaymentService : IPaymentService
    {
        ...
        public Dictionary<string, string> CreateSinglePaymentForm(string amount)
        {
            if (_configuration["paymentSiteId"] == null)
            {
                throw new Exception("Configuration key paymentSiteId not present or empty");
            }

            var siteId = _configuration["paymentSiteId"];
            ...
            paymentModel.Signature = SignPayment(paymentModel);
            _logger.LogInformation("Return the payment parameters");
            return CreatePaymentFormWithParameters(paymentModel);
        }

        public string SignPayment(PaymentFormModel paymentForm)
        {
            if (_configuration["paymentApiKey"] == null)
            {
                throw new Exception("Configuration key paymentApiKey not present or empty");
            }
            var apiKey = _configuration["paymentApiKey"];
